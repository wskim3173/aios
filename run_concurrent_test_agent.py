#!/usr/bin/env python3
# run_concurrent_test_agents.py
#
# - Local agent 목록에서 'test_agent' 디렉터리를 찾아 실행
# - 각 실행은 고유 임시 디렉터리(TMPDIR/XDG_CACHE_HOME)로 격리
# - Warm-up 1회 후 동시 실행
# - 결과는 JSONL 저장
...
TASK_TEMPLATE = "Find recent arXiv papers about AI scheduling and queuing, then summarize briefly. Req#{i}"

def preload_tools():
    # 네임스페이스 포함/미포함 둘 다 시도해도 OK
    try:
...
def copy_agent_dir(src: Path) -> Path:
    ...
    run_id = uuid4().hex[:8]
    dst = Path(tempfile.gettempdir()) / f"test_agent_dir_{run_id}"
    if dst.exists():
...
def _extract_text(result_obj) -> str:
    # TestAgent는 {"result": "..."} 형태를 반환하지만, 혹시 모를 변형에 대비
...
async def main(num_agents: int):
    ...
    # Pick test_agent directory
    test = next((a for a in local_agents if (a.get("name") or "").lower() == "test_agent"), None)
    if test is None:
        raise RuntimeError("test_agent not found among local agents.")

    test_dir = Path(test.get("path", ""))
    if not test_dir or not test_dir.is_dir():
        raise RuntimeError(f"test_agent path is invalid: {test_dir}")

    print(f"[INFO] Using test_agent at: {test_dir}")

    # Warm-up (build caches/deps once)
    print("[INFO] Warm-up run...")
    _ = await run_one_agent(test_dir, "WARMUP: say hi", agent_hub_url)
    ...
    async def guarded_run(task_str: str):
        ...
        agent_copy = copy_agent_dir(test_dir)
        async with sem:
            try:
    ...
    out_file = out_dir / f"test_agent_concurrent_{int(time.time())}.jsonl"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_agents", type=int, default=DEFAULT_NUM_AGENTS,
                        help="Number of concurrent test_agent runs.")
