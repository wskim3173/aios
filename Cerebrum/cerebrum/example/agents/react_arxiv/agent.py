...
class ReActAgent:
    ...
    def run(self, task_input: str):
        ...
        WORKER_PROMPTS = """
1. Worker name: "arxiv_search"

   Description:
   - Search academic papers on arXiv related to the given topic.
   - Returns a short summary of the top-k relevant papers.

   How to call:
   - Set `worker_name` to "arxiv_search".
   - Set `worker_params` to an object with the following field:
       - `query` (string): A concise English search query describing what you want to find.

   Example:
   {
       "worker_name": "arxiv_search",
       "worker_params": {
...
You have at most {self.max_steps} steps in total.  
Tool calls are expensive, so you must only call a worker when it is really necessary.

Your output MUST be a JSON object with:
- `observation` (string)
- `reasoning` (string)
- `worker_name` (string or null)
- `worker_params` (object or null)
...
- If you believe you ALREADY have enough information to answer the original task,
  ...
  - "worker_name": null
  - "worker_params": null
  and move to final answer extraction.
- Do NOT keep calling "arxiv_search" if you already know the explanation.
...
{{
    "observation": "I still need more academic references for this question.",
    "reasoning": "To proceed, I will search arxiv for relevant papers.",
    "worker_name": "arxiv_search",
    "worker_params": {{
        "query": "Why is the sky blue?"
    }}
