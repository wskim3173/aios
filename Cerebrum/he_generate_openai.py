...
def build_prompt(prompt):
    # HumanEval 프롬프트 그대로 쓰되, 함수 시그니처부터 시작하도록 유도
    return (
...
def main():
    ...
    with open(args.output_file, "w", encoding="utf-8") as f:
        for ex in ds:
            ...
            # JSONL로 저장 (HumanEval 포맷)
            f.write(json.dumps({"task_id": task_id, "completion": code}, ensure_ascii=False) + "\n")
            f.flush()
            cnt += 1
            time.sleep(0.4)  # rate 보호용(필요시)
